...
{
	cmsw* CMSW = reinterpret_cast<cmsw*>(tagLocation);

	if (ImGui::CollapsingHeader("AnyTag")) {
		ImGui::InputScalar("VTable Space", ImGuiDataType_U64, &CMSW->AnyTag_ent.vtable_space);

		if (ImGui::TreeNode("Internal Struct")) {
			ImGui::InputScalar("Global Tag ID", ImGuiDataType_U32, &CMSW->AnyTag_ent.internal_struct_ent.global_tag_ID);
			ImGui::InputScalar("Local Tag Handle", ImGuiDataType_U32, &CMSW->AnyTag_ent.internal_struct_ent.local_tag_handle);
			ImGui::TreePop();
		}
	}
